...
def compute_RMSEs(list_dfs, als_setting, get_tags=False):
    ...
    for i in range(len(df_list)):
        ...
        if get_tags:
            ...
            kmeans = KMeans().setK(10).setSeed(myseed).setFeaturesCol("features").setPredictionCol("cluster") # k=10
...
top_names,bot_names=[],[]
print("#############################################ALS setting 1####################################################\n")
RMSE_hot1,RMSE_cool1 = compute_RMSEs(df_list, als_setting1, get_tags=True) #find RMSEs and tags from Lab 7 ALS setting
print("#############################################ALS setting 2####################################################\n")
RMSE_hot2,RMSE_cool2 = compute_RMSEs(df_list, als_setting2)
print("ALS 1 hot users RMSE=",RMSE_hot1)
print("ALS 2 hot users RMSE=",RMSE_hot2)
print("ALS 1 cool users RMSE=",RMSE_cool1)
print("ALS 2 cool users RMSE=",RMSE_cool2)
...
ax.bar(ind + 0.0, RMSE_hot1, color = 'r', width=width, label='ALS1_hot')
ax.bar(ind + width/2, RMSE_hot2, color = 'g', width=width, label='ALS2_hot')
ax.bar(ind + width, RMSE_cool1, color = 'b', width=width, label='ALS1_cool')
ax.bar(ind + width*1.5, RMSE_cool2, color = 'y', width=width/1.5, label='ALS2_cool')

plt.ylabel('Root Mean Square Error')
plt.title('RMSE on hot and Cool Users for each Split')
plt.xticks(ind + width / 2, ('Split_1', 'Split_2', 'Split_3', 'Split_4', 'Split_5'))
plt.legend(bbox_to_anchor=(1.13, 1.05)) # Legend is displayed outside so that figure is easier to view
plt.savefig("""../Output/Q2fig_A.png"""
            , bbox_inches='tight')
...
print("Top and Bottom tags for 2 clusters")
