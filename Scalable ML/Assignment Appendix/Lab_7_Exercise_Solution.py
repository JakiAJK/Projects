...
ratings = spark.read.load('/data/acp21jka/ScalableML/Data/ml-latest-small/ratings.csv', format = 'csv', inferSchema = "true", header = "true").cache()
...
def split_by_row_index(df, num_partitions=5):
...
apic=[i.collect() for i in split_by_row_index(ratings)]
